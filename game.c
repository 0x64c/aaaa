...
void gameframe(void)
{
	...
	switch (gamemode)
	{
	case ZGM_MENU: gamemenu();
		break;
	case ZGM_CONFIG: gameconfig();
		break;
	case ZGM_SELECTOR: gameselector();
		break;

	case ZGM_LOGO: gamelogo();
		break;
...
//if (gameswitchdelay==0)
		{
			switch (playmode)
			{
			case ZPM_NONE: procplaycontrol();
				break;
			case ZPM_FLYINTRO: procintro();
				break;
			case ZPM_AAA: procgamecontrol();
				break;
			case ZPM_MOVIE: procmovie();
				break;

			case ZPM_AUTISM: procautism();
				break;
			case ZPM_ZVERYLE: proczveryle();
				break;
